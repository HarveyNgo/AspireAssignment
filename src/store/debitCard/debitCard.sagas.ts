...
function* getCurrency(action: Action<IGetCurrencyPayLoad>): any {
  const service = new DebitCardService();
  const result = yield service.getCurrency();

  if (result !== undefined) {
    yield put(DebitCardActions.getCurrencySuccess(result));
  }
}
...
export default function* watcherSaga() {
  ...
  yield takeLatest(DebitCardTypes.GET_CURRENCY, getCurrency);
}
